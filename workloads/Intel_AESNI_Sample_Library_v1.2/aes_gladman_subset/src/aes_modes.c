...
aligned_array(unsigned PFAY_LONG,    enc_gen_table, 12, 16) =    NEH_ENC_GEN_DATA;
aligned_array(unsigned PFAY_LONG,   enc_load_table, 12, 16) =   NEH_ENC_LOAD_DATA;
aligned_array(unsigned PFAY_LONG, enc_hybrid_table, 12, 16) = NEH_ENC_HYBRID_DATA;
aligned_array(unsigned PFAY_LONG,    dec_gen_table, 12, 16) =    NEH_DEC_GEN_DATA;
aligned_array(unsigned PFAY_LONG,   dec_load_table, 12, 16) =   NEH_DEC_LOAD_DATA;
aligned_array(unsigned PFAY_LONG, dec_hybrid_table, 12, 16) = NEH_DEC_HYBRID_DATA;
...
#define via_cwd(cwd, ty, dir, len) \
    unsigned PFAY_LONG* cwd = (dir##_##ty##_table + ((len - 128) >> 4))

#else

#define via_cwd(cwd, ty, dir, len)              \
    aligned_auto(unsigned PFAY_LONG, cwd, 4, 16);    \
    cwd[1] = cwd[2] = cwd[3] = 0;               \
