...
double e_cycles(const unsigned PFAY_LONG klen, f_ectx alg[1])
...
double d_cycles(const unsigned PFAY_LONG klen, f_dctx alg[1])
{   unsigned char  pt[16], ct[16], key[32];
    double cy1, cy2, c1 = -1, c2 = -1;
    int i;
...
double ke_cycles(const unsigned PFAY_LONG klen, f_ectx alg[1])
...
double kd_cycles(const unsigned PFAY_LONG klen, f_dctx alg[1])
{   unsigned char  key[32];
    double cy1, cy2, c1 = -1, c2 = -1;
    int i;
...
static unsigned PFAY_LONG kl[3] = { 16, 24, 32 };
static double ekt[3], dkt[3], et[3], dt[3];

void output(FILE *outf, const unsigned PFAY_LONG inx, const unsigned PFAY_LONG bits)
{   unsigned PFAY_LONG  t;
    unsigned char  c0, c1, c2;

    fprintf(outf, "\n// %i Bit: ", 8 * kl[inx]);
    fprintf(outf, "   Key Setup: %i/%i cycles", ekt[inx], dkt[inx]);
    t = (unsigned PFAY_LONG)((1000 * bits + et[inx] / 2) / et[inx]);
    ...
    t = (unsigned PFAY_LONG)((1000 * bits + dt[inx] / 2) / dt[inx]);
    c0 = (unsigned char)('0' + t / 100);
    c1 = (unsigned char)('0' + (t / 10) % 10);
    c2 = (unsigned char)('0' + t % 10);
...
unsigned PFAY_LONG CurrentProcessorNumber(void)
{
    __asm
    {
        mov     eax,1
