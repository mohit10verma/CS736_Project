...
{
	aes_params* aes_args = (aes_params*)args;
	char* bmark_file_name = aes_args->bmark_file_name;
	int bmark_key_size = aes_args->bmark_key_size; 
	int bmark_file_size = aes_args->bmark_file_size; 
	int bmark_encrypt = aes_args->bmark_encrypt; 
	int bmark_loops = aes_args->bmark_loops; 
	int outer_loops = aes_args->outer_loops;
	char *fl_in={"tmp.txt"};
	char *fl_input, *fl_vary={"tmp_in.txt"};
	//char *fl_in={"\\pfay\\bigfile.dat"};
	char *fl_enc={"tmp_enc.txt"}, *fl_dec={"tmp_dec.txt"};
	unsigned char output[32], doutput[32];
	int i, in_sz, oper, fileSize=0;
	unsigned int crypt_method[2];
	//char bmark_file_name[1024] = {"bigfile.txt"};
	int key_size_in_bytes=0;
   ...
   if (my_optind < argc) {
    ...
    int bmark_decrypt = bmark_encrypt;
    ...
    aes_decrypt(USE_iAES, output, doutput, gkey, 32);

	printf("output is 0x");
	for(i=0; i < 16; i++) {printf("%.2x", (unsigned int)(doutput[i]));}
	printf("\n");
	printf("expect is 0x");
	for(i=0; i < 16; i++) {printf("%.2x", (unsigned int)(test_input[i]));}
	printf("\n");
	for(i=0; i < 16; i++) { 
		if(test_input[i] != doutput[i]) { 
			printf("---------------mismatch at char %d is 0x%.2x should be 0x%.2x\n",
				i, (unsigned int)doutput[i], (unsigned int)test_input[i]);
			exit(2);
		}
	}
