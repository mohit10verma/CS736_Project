...
#define KB 1024
#define MB (1024*KB)
#define BLOSC_FIBER_STACK_SIZE 1024000
#include "fiber.h"
...
struct blosc_context {
  ...
  int32_t end_threads;
  //pthread_t threads[BLOSC_MAX_THREADS];//TODO
  fiber_t *threads[BLOSC_MAX_THREADS];
  int32_t tids[BLOSC_MAX_THREADS];
  //pthread_mutex_t count_mutex;
  fiber_mutex_t count_mutex;
  #ifdef _POSIX_BARRIERS_MINE
  //pthread_barrier_t barr_init;
  //pthread_barrier_t barr_finish;
  fiber_barrier_t barr_init;
  fiber_barrier_t barr_finish;
  #else
  int32_t count_threads;
  //pthread_mutex_t count_threads_mutex;
  //pthread_cond_t count_threads_cv;
  fiber_mutex_t count_threads_mutex;
  fiber_cond_t count_threads_cv;
  #endif
  #if !defined(_WIN32)
  //pthread_attr_t ct_attr;            /* creation time attrs for threads */ TODO: No pthread_attr_t type
  #endif
...
static fiber_mutex_t global_comp_mutex;
...
/* Wait until all threads are initialized */    //changed from pthread here TODO: CHECK PTHREAD_BARRIER_SERIAL_THREAD
#ifdef _POSIX_BARRIERS_MINE
#define WAIT_INIT(RET_VAL, CONTEXT_PTR)  \
  rc = fiber_barrier_wait(&CONTEXT_PTR->barr_init); \
  if (rc != 0 && rc != FIBER_BARRIER_SERIAL_FIBER) { \
    printf("Could not wait on barrier (init): %d\n", rc); \
    return((RET_VAL));                            \
  }
#else
#define WAIT_INIT(RET_VAL, CONTEXT_PTR)   \
  fiber_mutex_lock(&CONTEXT_PTR->count_threads_mutex); \
  if (CONTEXT_PTR->count_threads < CONTEXT_PTR->numthreads) { \
    CONTEXT_PTR->count_threads++;  \
    fiber_cond_wait(&CONTEXT_PTR->count_threads_cv, &CONTEXT_PTR->count_threads_mutex); \
  } \
  else { \
    fiber_cond_broadcast(&CONTEXT_PTR->count_threads_cv); \
  } \
  fiber_mutex_unlock(&CONTEXT_PTR->count_threads_mutex);
#endif

/* Wait for all threads to finish */      //changed from pthread here TODO: CHECK PTHREAD_BARRIER_SERIAL_THREAD
#ifdef _POSIX_BARRIERS_MINE
#define WAIT_FINISH(RET_VAL, CONTEXT_PTR)   \
  rc = fiber_barrier_wait(&CONTEXT_PTR->barr_finish); \
  if (rc != 0 && rc != FIBER_BARRIER_SERIAL_FIBER) { \
                   ...
                   const void *src, void *dest, size_t destsize)
{
  ...
  fiber_mutex_lock(&global_comp_mutex);
  ...
  fiber_mutex_unlock(&global_comp_mutex);
...
static void *t_blosc(void *ctxt)
{
  ...
  while(1)
  {
    ...
    if (compress && !(flags & BLOSC_MEMCPYED)) {
      ...
      fiber_mutex_lock(&context->parent_context->count_mutex);
      context->parent_context->thread_nblock++;
      nblock_ = context->parent_context->thread_nblock;
      fiber_mutex_unlock(&context->parent_context->count_mutex);
      tblock = nblocks;
    }
    else {
...
static int init_threads(struct blosc_context* context)
{
  ...
  fiber_mutex_init(&context->count_mutex);
...
#ifdef _POSIX_BARRIERS_MINE
  fiber_barrier_init(&context->barr_init, context->numthreads+1);
  fiber_barrier_init(&context->barr_finish, context->numthreads+1);
#else
  //pthread_mutex_init(&context->count_threads_mutex, NULL);
  //pthread_cond_init(&context->count_threads_cv, NULL);
  fiber_mutex_init(&context->count_threads_mutex);
  fiber_cond_init(&context->count_threads_cv);
  context->count_threads = 0;      /* Reset threads counter */
#endif

#if !defined(_WIN32)
  /* Initialize and set thread detached attribute */
  //pthread_attr_init(&context->ct_attr);//TODO: CHECK
  //pthread_attr_setdetachstate(&context->ct_attr, PTHREAD_CREATE_JOINABLE);//TODO: CHECK
#endif
...
#if !defined(_WIN32)
    //rc2 = pthread_create(&context->threads[tid], &context->ct_attr, t_blosc, (void *)thread_context);//TODO
    context->threads[tid] = fiber_create(BLOSC_FIBER_STACK_SIZE, t_blosc, (void *)thread_context);
#else
    //rc2 = pthread_create(&context->threads[tid], NULL, t_blosc, (void *)thread_context);
    context->threads[tid] = fiber_create(BLOSC_FIBER_STACK_SIZE, t_blosc, (void *)thread_context);
#endif
    rc2 = 0;//TODO: HARDCODED
    if (rc2) {
...
void blosc_init(void)
{
  ...
  fiber_mutex_init(&global_comp_mutex);
...
void blosc_destroy(void)
{
  ...
  fiber_mutex_destroy(&global_comp_mutex);
}
...
int blosc_release_threadpool(struct blosc_context* context)
{
  ...
  if (context->threads_started > 0)
  {
    ...
    for (t=0; t<context->threads_started; t++) {
      ...
      rc2 = fiber_join(context->threads[t], &status);
      if (rc2) {
    ...
    fiber_mutex_destroy(&context->count_mutex);

    /* Barriers */
  #ifdef _POSIX_BARRIERS_MINE
      //pthread_barrier_destroy(&context->barr_init);
      //pthread_barrier_destroy(&context->barr_finish);
    fiber_barrier_destroy(&context->barr_init);
    fiber_barrier_destroy(&context->barr_finish);
  #else
      //pthread_mutex_destroy(&context->count_threads_mutex);
      //pthread_cond_destroy(&context->count_threads_cv);
    fiber_mutex_destroy(&context->count_threads_mutex);
    fiber_cond_destroy(&context->count_threads_cv);
  #endif

      /* Thread attributes */
  #if !defined(_WIN32)
      //pthread_attr_destroy(&context->ct_attr);//TODO: CHECK
  #endif
