...
#undef USE_STACKSIZE_CUSTOMIZATION
//
// GLOBALS
//
...
static fiber_mutex_t *OutMutex=NULL;
static fiber_mutex_t *ProducerDoneMutex=NULL;
/*static fiber_mutex_t ErrorHandlerMutex = PTHREAD_MUTEX_INITIALIZER;
static fiber_mutex_t TerminateFlagMutex = PTHREAD_MUTEX_INITIALIZER;
static fiber_mutex_t ProgressIndicatorsMutex = PTHREAD_MUTEX_INITIALIZER;*/
static fiber_mutex_t ErrorHandlerMutex;
static fiber_mutex_t TerminateFlagMutex;
static fiber_mutex_t ProgressIndicatorsMutex;
static fiber_cond_t *notTooMuchNumBuffered;
/*static fiber_cond_t TerminateCond = PTHREAD_COND_INITIALIZER;
static fiber_cond_t OutBufferHeadNotEmpty = PTHREAD_COND_INITIALIZER;
static fiber_cond_t ErrStateChangeCond = PTHREAD_COND_INITIALIZER;*/
static fiber_cond_t TerminateCond;
static fiber_cond_t OutBufferHeadNotEmpty;
static fiber_cond_t ErrStateChangeCond;
//static fiber_attr_t ChildThreadAttributes;			//TODO - Remove attr_init no such type
#define PBZIP_STACK_SIZE  	20000
...
static fiber_t *SignalHandlerThread;
static fiber_t *TerminatorThread;
...
int safe_cond_timed_wait(fiber_cond_t *cond, fiber_mutex_t *mutex, int seconds, const char *caller = "safe_cond_timed_wait");
 ...
 */
int safe_cond_timed_wait(fiber_cond_t *cond, fiber_mutex_t *mutex, int seconds, const char *caller)
{
...
inline void syncSetTerminateFlag(int newValue)
{
	...
	if (terminateFlag != 0)
	{
		...
		fiber_cond_signal(&TerminateCond);
		safe_mutex_unlock(&TerminateFlagMutex);

		// wake up all other possibly blocked on cond threads
		safe_mutex_lock(OutMutex);
		fiber_cond_broadcast(notTooMuchNumBuffered);
		safe_mutex_unlock(OutMutex);
		if (FifoQueue != NULL)
		{
			safe_mutex_lock(FifoQueue->mut);
			fiber_cond_broadcast(FifoQueue->notFull);
			fiber_cond_broadcast(FifoQueue->notEmpty);
...
int setupSignalHandling()
{
	...
	if (ret == 0)
	{
		...
		SignalHandlerThread = fiber_create(PBZIP_STACK_SIZE, signalHandlerProc, NULL);
	}
...
int setupTerminator()
{
	...
	TerminatorThread = fiber_create(PBZIP_STACK_SIZE, terminatorThreadProc, NULL);
	return 0;
}
...
void* terminatorThreadProc(void* arg)
{
	...
	{
		ret = fiber_cond_wait(&TerminateCond, &TerminateFlagMutex);
	}

	// Successfull end
	if (finishedFlag != 0)
	{
		ret = fiber_mutex_unlock(&TerminateFlagMutex);
		return NULL;
	}
...
void cleanupAndQuit(int exitCode)
{
	...
	fiber_mutex_unlock(&ErrorHandlerMutex);
...
{
	// initialize mutexes
	OutMutex = new(std::nothrow) fiber_mutex_t;
	// make sure memory was allocated properly
	if (OutMutex == NULL)
	{
	...
	fiber_mutex_init(OutMutex);

	ProducerDoneMutex = new(std::nothrow) fiber_mutex_t;
	// make sure memory was allocated properly
	...
	fiber_mutex_init(ProducerDoneMutex);
...
void mutexesDelete()
{
	...
	if (ProducerDoneMutex != NULL)
	{
		fiber_mutex_destroy(ProducerDoneMutex);
...
queue *queueInit(int queueSize)
{
	...
	q->mut = NULL;
	q->mut = new(std::nothrow) fiber_mutex_t;
	if (q->mut == NULL)
		return NULL;
	//fiber_mutex_init(q->mut, NULL);
	fiber_mutex_init(q->mut);

	q->notFull = NULL;
	q->notFull = new(std::nothrow) fiber_cond_t;
	if (q->notFull == NULL)
		return NULL;
	//fiber_cond_init(q->notFull, NULL);
	fiber_cond_init(q->notFull);

	q->notEmpty = NULL;
	q->notEmpty = new(std::nothrow) fiber_cond_t;
	if (q->notEmpty == NULL)
		return NULL;
	//fiber_cond_init(q->notEmpty, NULL);
	fiber_cond_init(q->notEmpty);

	q->consumers = NULL;
	q->consumers = new(std::nothrow) fiber_t* [queueSize];
	if (q->consumers == NULL)
		return NULL;

	notTooMuchNumBuffered = NULL;
	notTooMuchNumBuffered = new(std::nothrow) fiber_cond_t;
...
void queueDelete (queue *q)
{
	...
	if (notTooMuchNumBuffered != NULL)
	{
		fiber_cond_destroy(notTooMuchNumBuffered);
...
int pbzip2_start(void* arg)
{
	...
	fiber_mutex_init(&ProgressIndicatorsMutex);
		...
		if (decompress == 1)
		{
			...
			if (noThreads == 0)
			{
				...
				for (i=0; (int)i < numCPU; i++)
				{
					...
					fifo->consumers[i] = fiber_create(PBZIP_STACK_SIZE, consumer_decompress, fifo);
					ret = 0;
					if (ret != 0)
					{
						ErrorContext::getInstance()->saveError();
				...
				output = fiber_create(PBZIP_STACK_SIZE, fileWriter, (void*)outFilename.c_str());
				ret = 0;
				if (ret != 0)
				{
					handle_error(EF_EXIT, 1,
			...
			if (noThreads == 0)
			{
				...
				for (i=0; (int)i < numCPU; i++)
				{
					...
					if (ret != 0)
					{
						...
						fiber_join(TerminatorThread, NULL);
						return 1;
					}
				}
	...
	//ret = pthread_kill(SignalHandlerThread, SIG_HANDLER_QUIT_SIGNAL);//TODO - check
	ret = 0;
	if (ret != 0)
	{
	...
	{
		//ret = fiber_join(SignalHandlerThread, NULL);
		ret = 0;
		if (ret != 0)
		{
