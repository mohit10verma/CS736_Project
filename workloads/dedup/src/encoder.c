...
static void write_chunk_to_file(int fd, chunk_t *chunk) {
  ...
  fiber_mutex_lock(&chunk->header.lock);
  while(chunk->header.state == CHUNK_STATE_UNCOMPRESSED) {
    fiber_cond_wait(&chunk->header.update, &chunk->header.lock);
  }
  ...
  fiber_mutex_unlock(&chunk->header.lock);
}
#else
...
void sub_Compress(chunk_t *chunk) {
    ...
    fiber_cond_broadcast(&chunk->header.update);
    fiber_mutex_unlock(&chunk->header.lock);
#endif //ENABLE_PTHREADS
...
#ifdef ENABLE_PTHREADS
  fiber_mutex_t *ht_lock = hashtable_getlock(cache, (void *)(chunk->sha1));
  fiber_mutex_lock(ht_lock);
#endif
...
#ifdef ENABLE_PTHREADS
    fiber_mutex_init(&chunk->header.lock);
    fiber_cond_init(&chunk->header.update);
#endif
...
void Encode(config_t * _conf) {
   ...
   */
  fiber_t * threads_anchor[MAX_THREADS],
  ...
  threads_process = fiber_create(24000, &Fragment, &data_process_args);
  ...
  for (i = 0; i < conf->nthreads; i ++) {
     ...
     threads_anchor[i] = fiber_create( 24000, &FragmentRefine, &anchor_thread_args[i]);
  }

  struct thread_args chunk_thread_args[conf->nthreads];
  for (i = 0; i < conf->nthreads; i ++) {
    chunk_thread_args[i].tid = i;
    threads_chunk[i] = fiber_create(24000, &Deduplicate, &chunk_thread_args[i]);
  }

  struct thread_args compress_thread_args[conf->nthreads];
  for (i = 0; i < conf->nthreads; i ++) {
    compress_thread_args[i].tid = i;
    threads_compress[i] = fiber_create(24000, &Compress, &compress_thread_args[i]);
  }
  ...
  threads_send = fiber_create(24000, &Reorder, &send_block_args);
  ...
  //join all threads 
  fiber_join(threads_process, NULL);
  for (i = 0; i < conf->nthreads; i ++)
    fiber_join(threads_anchor[i], (void **)&threads_anchor_rv[i]);
  for (i = 0; i < conf->nthreads; i ++)
    fiber_join(threads_chunk[i], (void **)&threads_chunk_rv[i]);
  for (i = 0; i < conf->nthreads; i ++)
    fiber_join(threads_compress[i], (void **)&threads_compress_rv[i]);
  fiber_join(threads_send, NULL);
