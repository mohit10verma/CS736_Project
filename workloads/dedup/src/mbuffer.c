...
typedef fiber_spinlock_t fiber_lock_t;
#define PTHREAD_LOCK_INIT(l) fiber_spinlock_init(l)
#define PTHREAD_LOCK_DESTROY(l) fiber_spinlock_destroy(l)
#define PTHREAD_LOCK(l) fiber_spinlock_lock(l)
#define PTHREAD_UNLOCK(l) fiber_spinlock_unlock(l)
#else
typedef fiber_mutex_t fiber_lock_t;
#define PTHREAD_LOCK_INIT(l) fiber_mutex_init(l)
#define PTHREAD_LOCK_DESTROY(l) fiber_mutex_destroy(l)
#define PTHREAD_LOCK(l) fiber_mutex_lock(l)
#define PTHREAD_UNLOCK(l) fiber_mutex_unlock(l)
...
int mbuffer_system_init() {
  ...
  assert(locks==NULL);
  locks = malloc(NUMBER_OF_LOCKS * sizeof(fiber_lock_t));
  if(locks==NULL) return -1;
  for(i=0; i<NUMBER_OF_LOCKS; i++) {
    if(PTHREAD_LOCK_INIT(&locks[i]) != 1) {
      int j;
      for(j=0; j<i; j++) {
