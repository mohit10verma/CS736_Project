...
/*--------------------------------------------------------------------------*/
typedef struct dedup_param{
    int dedup_argc;
    char **dedup_argv;
}dedup_params;

//change main
int dedup_start(void *arg) {
  dedup_params* param = (dedup_params*)arg;
  int argc = param->dedup_argc;
  char **argv = param->dedup_argv;
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
 ...
 if (conf->nthreads != 1){
  ...
  //fiber_manager_init(conf->nthreads); //changed!!!!!!!!!!!!
