...
#ifdef ENABLE_PTHREADS
  fiber_mutex_init(&que->mutex);
  fiber_cond_init(&que->notEmpty);
  fiber_cond_init(&que->notFull);
#endif
...
#ifdef ENABLE_PTHREADS
  fiber_mutex_destroy(&que->mutex);
  fiber_cond_destroy(&que->notEmpty);
  fiber_cond_destroy(&que->notFull);
#endif
...
#ifdef ENABLE_PTHREADS
  fiber_mutex_lock(&que->mutex);
#endif
  que->nTerminated++;
  assert(que->nTerminated <= que->nProducers);
#ifdef ENABLE_PTHREADS
  if(queue_isTerminated(que)) fiber_cond_broadcast(&que->notEmpty);
  fiber_mutex_unlock(&que->mutex);
#endif
}
...
int queue_dequeue(queue_t *que, ringbuffer_t *buf, int limit) {
  ...
  while (ringbuffer_isEmpty(&que->buf) && !queue_isTerminated(que)) {
    fiber_cond_wait(&que->notEmpty, &que->mutex);
  }
#endif
...
#ifdef ENABLE_PTHREADS
  if(i>0) fiber_cond_signal(&que->notFull);
  fiber_mutex_unlock(&que->mutex);
#endif
  return i;
}
...
int queue_enqueue(queue_t *que, ringbuffer_t *buf, int limit) {
  ...
  while (ringbuffer_isFull(&que->buf))
    fiber_cond_wait(&que->notFull, &que->mutex);
#else
  assert(!queue_isTerminated(que));
#endif
...
#ifdef ENABLE_PTHREADS
  if(i>0) fiber_cond_signal(&que->notEmpty);
  fiber_mutex_unlock(&que->mutex);
#endif
  return i;
}
